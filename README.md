...
![img_25.png](src%2Fimages%2Fimg_25.png)  
`git tag`  
![img.png](img.png)  
посмотреть теги  
![img_26.png](src%2Fimages%2Fimg_26.png)  
переключаться - аккуратно, изменения потерять нельзя, а запутаться - можно легко (detached head например)  
...
![img_27.png](src%2Fimages%2Fimg_27.png)  
...
(если вкратце, оба пути хороши, но с гит ребейзом - можно долго и очень много ребейзить каждое изменение в каждом конфликте, что очень может утомить)  
...
Хотя, можно и совмещать оба подхода - вполне легко  
...
`git stash apply` // `git stash apply stash@{n}` (`git stash list `для номера)
